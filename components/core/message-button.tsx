...
export function MessageButton({
  ...
  const streamlitResult = useMemo(() =>
    toolInvocations?.find(
      invocation => isToolResult(invocation) && invocation.toolName === 'create_streamlit_app'
  ...
  const currentToolState = useMemo(() => {
    ...
    return {
      ...
      state: loadingStates[toolCall.toolCallId]?.isLoading ? 'loading' : 'complete'
    }
  ...
  }, [currentToolState, object, result, streamlitResult, messageId, onObjectClick, onToolResultClick, onCodeClick, setCodeViewOpen])
  ...
  const renderButtonContent = () => {
    const icon = (() => {
      if (isLoading || currentToolCall.state === 'streaming' || currentToolCall.state === 'delta') {
        return <Loader2 strokeWidth={2} className="h-4 w-4 text-[#FF8800] animate-spin" />
      }
    ...
    const subtitle = (() => {
      if (currentToolCall.state === 'streaming' || currentToolCall.state === 'delta') {
        return "Please wait..."
      }
