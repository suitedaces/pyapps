    ...
    const renderTable = (data: string[][], startIndex: number = 0) => (
        <table ref={tableRef} className="w-full border-collapse table-auto">
            <thead className="sticky top-0 bg-gray-100 z-10">
                <tr>
                    <th className="border-r border-gray-300 px-2 py-2 text-left font-bold">
                        #
                    </th>
                    {analysis.columns.map((column, index) => (
                        <th
                            key={index}
                            className="border-r border-gray-300 px-4 py-2 text-left font-bold whitespace-nowrap overflow-hidden"
                            style={{ maxWidth: '200px' }}
                        >
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <span className="block truncate">
                                        {truncateText(column.name, 20)}
                                    </span>
                                </TooltipTrigger>
                                <TooltipContent>
                                    <p>{column.name}</p>
            ...
            <tbody>
                {data.map((row, rowIndex) => (
                    ...
                    >
                        <td className="border-r border-gray-300 px-2 py-2 text-gray-500 text-sm">
                            {startIndex + rowIndex + 1}
                        </td>
                        {row.map((cell, cellIndex) => (
                            <td
                                key={cellIndex}
                                className="border-r border-gray-300 px-4 py-2 overflow-hidden text-ellipsis"
                                style={{ maxWidth: '200px' }}
                            >
                                <Tooltip>
                                    <TooltipTrigger asChild>
