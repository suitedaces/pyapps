...
export async function updateChat(chatId: string, updates: Partial<Database['public']['Tables']['chats']['Update']>) {
  const { data, error } = await supabase
    .from('chats')
    .update(updates)
    .eq('id', chatId)
...
export async function updateApp(appId: string, updates: Partial<Database['public']['Tables']['apps']['Update']>) {
  const { data, error } = await supabase
    .from('apps')
    .update(updates)
    .eq('id', appId)
...
export async function createAppVersion(appId: string, versionNumber: number, code: string) {
  const { data, error } = await supabase
    .from('app_versions')
...
export async function updateUserUsage(userId: string, updates: Partial<Database['public']['Tables']['usage_limits']['Update']>) {
  const { data, error } = await supabase
    .from('usage_limits')
    .update(updates)
    .eq('user_id', userId)
...
export async function updateAppPublicStatus(appId: string, versionId: string, publicId: string) {
  const { data, error } = await supabase
    .rpc('update_app_public_status', {
      p_app_id: appId,
...
export async function getExpiredFiles(cutoffDate?: string) {
  const { data, error } = await supabase
    .rpc('get_expired_files', { cutoff_date: cutoffDate })

  if (error) throw error
  return data
}

export async function getLatestMessagesByChat(userId: string, limit: number = 10) {
  const { data, error } = await supabase
    .rpc('get_latest_messages_by_chat', { p_user_id: userId, p_limit: limit })
...
export async function getUserTotalTokens(userId: string) {
  const { data, error } = await supabase
    .rpc('get_user_total_tokens', { p_user_id: userId })
